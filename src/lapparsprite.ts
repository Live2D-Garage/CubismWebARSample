...
export default class LAppARSprite {
  constructor(private _modelOption: ModelOption) {
    ...
    this.doUpdate(ARMarkerEvent.Detect);
  }
  ...
  release(scene: THREE.Scene) {
    ...
    geometry instanceof THREE.BufferGeometry && geometry.dispose();
  }
  ...
  onHitTest(hitResults: THREE.Intersection[]) {
    if (this.isVisible()) {
      hitResults.forEach((hitResult) => {
        this._hitAreas.forEach((value, key) => {
          if(hitResult.object.children.length!=0){
            console.log(hitResult.object.children[0]);
            hitResult.object.parent.add(hitResult.object.children[0])
            hitResult.object.remove(hitResult.object);
            if (value.action) {
              value.action();
            }
          }
